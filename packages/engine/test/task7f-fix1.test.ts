...
import {ready,act,pass,closeWindow,passReclaims} from './combat-helpers.js';
...
describe('Task7f review fix1',()=>{
 ...
 it('R2 excludes a revived same-batch victim at projection, acceptance and accepted-action resolution',()=>{
  ...
  top(s,'a2-p01-r1c1');const actionId=s.windows!.at(-1)!.continuation.id;s=act(s,'D',{type:'PLAY_REACTION',cardInstanceId:fate,mode:'cancel',targetActionId:actionId});s=closeWindow(s,[1]);s=closeWindow(s);s=act(s,'A',{type:'CHOOSE_REVIVAL',revive:true});s=act(s,'A',{type:'PASS_SETUP'});s=passReclaims(s);s=closeWindow(s);s=passReclaims(s);s=closeWindow(s);s=passReclaims(s);s=pass(s);
  ...
  s=act(s,'C',{type:'PLAY_DEATH_GIFT',cardInstanceId:good,giftCardInstanceId:giftC,targetId:'D'});const action=s.actions![s.windows!.at(-1)!.continuation.id]!;if(action.lifecycleEffect?.kind!=='gift')throw Error('NOT_GIFT');action.lifecycleEffect.targetId='A';action.targetIds=['A'];s=closeWindow(s);s=passReclaims(s);expect(s.players.A!.hand).not.toContain(giftC);expect(s.players.C!.hand).toContain(giftC);expect(s.discard).toContain(good);expect(allCardInstanceIds(s)).toHaveLength(220);
 });
});
