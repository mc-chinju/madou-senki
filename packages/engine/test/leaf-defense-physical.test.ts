...
it.each([[1,1,true,5],[6,6,false,0]] as const)('ordinary actual attacker %s %s success=%s leaves body %s without a counter attack',(d1,d2,success,damage)=>{
 let s=makeLeafDefenseScenario('leaf-ordinary',players);s=settle(act(s,'B',command('leaf-ordinary')),[d1,d2]);expect(s.rolls!.find(r=>r.purpose==='technique-check')).toMatchObject({rollerId:'A',modifier:-1,threshold:5,success,faces:[d1,d2]});expect([s.players.A!.damage,s.players.B!.damage]).toEqual([0,damage]);expect(s.discard.filter(id=>id===card)).toHaveLength(1);
});
...
it.each(['leaf-three-ordinary','leaf-three-dedicated'] as const)('%s actual chanted three-hit two-target Shin attack loses only B first hit',scenario=>{
 let s=makeLeafDefenseScenario(scenario,players);expect(Object.values(s.groups!)[0]!.targets.map(t=>t.hits.length)).toEqual([3,3]);s=settle(act(s,'B',command(scenario)));expect([s.players.A!.damage,s.players.B!.damage,s.players.C!.damage]).toEqual([0,14,21]);expect(s.discard.filter(id=>id===card)).toHaveLength(1);expect(s.discard.filter(id=>id==='a2-p10-r1c3')).toHaveLength(1);expect(s.rolls!.filter(r=>r.purpose==='technique-check'&&r.rollerId==='A')).toHaveLength(leafDefenseMode(scenario).dedicated?0:1);
});
it.each(rows)('%s actual Fate cancellation pays defense but leaves the original attack intact',scenario=>{
 let s=act(makeLeafDefenseScenario(scenario,players),'B',command(scenario));const id=Object.values(s.actions!).find(a=>a.cardInstanceId===card)!.id;expect(s.windows!.at(-1)!.participants[s.windows!.at(-1)!.cursor]).toBe('C');s=settle(act(s,'C',{type:'PLAY_REACTION',cardInstanceId:'a2-p02-r2c3',mode:'cancel',targetActionId:id}));expect(s.players.B!.damage).toBe(5);expect((s.rolls??[]).filter(r=>r.purpose==='technique-check')).toEqual([]);expect(s.discard.filter(id=>id===card)).toHaveLength(1);
});
it('ordinary actual enemy success allows a different defense while spent Leaf cannot be retried',()=>{
 let s=act(makeLeafDefenseScenario('leaf-ordinary',players),'B',command('leaf-ordinary'));for(let n=0;n<200&&!(s.windows?.at(-1)?.kind==='normal-defense'&&s.discard.includes(card));n++)s=pass(s);expect(s.rolls!.find(r=>r.purpose==='technique-check')!.success).toBe(true);reject(s,'B',command('leaf-ordinary'));s=finish(act(s,'B',{type:'PLAY_DEFENSE',cardInstanceId:'a2-p05-r3c1',dedicated:false}));expect(s.players.B!.damage).toBe(0);expect(s.discard.filter(id=>id===card)).toHaveLength(1);
});
...
it.each(rows)('%s actual failed own use check prevents even dedicated automatic negation',scenario=>{
 let s=act(makeLeafDefenseScenario(scenario,players,{level:2}),'B',command(scenario));s=until(s,'before-roll');expect(s.rolls!.at(-1)).toMatchObject({rollerId:'B',purpose:'excess-level'});for(let n=0;n<400&&s.windows?.length;n++){const r=s.rolls?.at(-1);s=pass(s,r?.rollerId==='B'&&r.purpose==='excess-level'&&r.stage==='before-roll'?[6,6]:[1,1]);}expect(s.rolls!.find(r=>r.purpose==='excess-level')!.success).toBe(false);expect(s.rolls!.filter(r=>r.purpose==='technique-check')).toEqual([]);expect(s.players.B!.damage).toBe(5);expect(s.discard.filter(id=>id===card)).toHaveLength(1);
});
