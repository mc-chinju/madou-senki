...
it.each(['before-damage','after-damage','before-snapshot','after-snapshot'] as const)('WhiteSword independent freezes with source disabled %s',boundary=>{
 ...
 s=until(s,'follower-start');s=closeWindow(s);const d=group(s).targets[0]!.followerDefense![0]!;expect(d.hits[0]).toMatchObject(boundary==='after-snapshot'?{outcome:'attribute-destroyed',hpReduction:0}:{outcome:'lower-destroyed',hpReduction:0});expect(s.discard).toContain(f);s=finish(s);
});
...
it.each(cases)('%s real ordinary Heavenly Hundred Slash freezes shared destruction and physical HP independently on three hits',(owner,id)=>{
 ...
 expect(t.hits.map(h=>h.damage)).toEqual(id===WIND?[7,7,7]:[2,2,2]);expect(s.discard).toContain(f);expect(s.discard).toContain(rear);s=finish(s);
});
