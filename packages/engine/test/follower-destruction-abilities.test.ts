...
it.each([
 ...
 if(outcome==='attribute-destroyed'||outcome==='level-destroyed'){expect(d).toMatchObject({defeated:true,revivalForbidden:true,hits:[{hpReduction:0}]});expect(d.morale).toBeUndefined();expect(s.rolls?.some(r=>r.purpose==='follower-morale')??false).toBe(false);expect(s.discard).toContain(f);}
 s=finish(s);
});
...
it.each(['before-damage','after-damage','before-snapshot','after-snapshot'] as const)('WhiteSword independent freezes with source disabled %s',boundary=>{
 ...
 s=until(s,'follower-start');s=closeWindow(s);const d=group(s).targets[0]!.followerDefense![0]!;expect(d.hits[0]).toMatchObject(boundary==='after-snapshot'?{outcome:'attribute-destroyed',hpReduction:0}:{outcome:'lower-destroyed',hpReduction:0});expect(s.resolution).toContain(f);s=finish(s);expect(s.discard).toContain(f);
});
...
it.each(cases)('%s real ordinary Heavenly Hundred Slash freezes shared destruction and physical HP independently on three hits',(owner,id)=>{
 ...
 expect(t.hits.map(h=>h.damage)).toEqual(id===WIND?[7,7,7]:[2,2,2]);expect(s.resolution).toContain(f);expect(s.resolution).toContain(rear);s=finish(s);expect(s.discard).toContain(f);expect(s.discard).toContain(rear);
});
...
it('Royal guard child pauses physical disposal after reached Lancaster dragon destruction',()=>{
 ...
 expect(s.players.B!.followers).toContainEqual({cardInstanceId:front,revealed:true});expect(s.discard).not.toContain(front);expect(s.groups![parentId]!.targets[0]!.followerDefense![0]!).toMatchObject({defeated:true,revivalForbidden:true,hits:[{outcome:'attribute-destroyed',hpReduction:0}]});
 const child=Object.values(s.groups!).find(g=>g.attackerId==='B')!;expect(child.technique.destroyFollowerAttributes).toEqual(['竜']);expect(s.actions![child.actionId]!.effectSourceCardInstanceId).not.toBe(guard);suppress(s);s=finish(s);expect(s.discard).toContain(front);expect(s.discard).toContain(returnedDragon);expect(s.players.B!.followers.some(f=>f.cardInstanceId===guard)).toBe(true);expect(s.players.A!.damage).toBe(4);
});
