...
it('loads a new copy of the actual four-player JSON fixture for save/resume', () => {
  ...
  expect(transition(s, { actorId: s.pending!.participantIds[0]!, command: { type: 'PASS_SETUP' } }, entropy())).toEqual(transition(freshGame(), { actorId: 'A', command: { type: 'PASS_SETUP' } }, entropy()));
