...
import {makeFireMagicScenario,fireMagicMode,type FireMagicScenario} from '../../../apps/worker/test/fixtures/fire-magic-scenarios.js';
...
it.each(rows)('%s actual Fate cancellation pays once without magic damage or dedicated leftovers',scenario=>{
 const m=fireMagicMode(scenario);let s=act(makeFireMagicScenario(scenario,players),'A',command(scenario));const id=Object.values(s.actions!).find(a=>a.cardInstanceId===m.card)!.id;s=pass(s);s=pass(s);s=act(s,'C',{type:'PLAY_REACTION',cardInstanceId:'a2-p02-r2c3',mode:'cancel',targetActionId:id});s=finish(s);expect([s.players.B!.damage,s.players.C!.damage]).toEqual([0,0]);expect(s.discard.filter(id=>id===m.card)).toHaveLength(1);expect(s.rolls??[]).toEqual([]);
});
