import {createGame,transition,viewFor,type GameState, moveToDiscard } from '@madou/engine';
...
export function makeSwordShuffleScenario(players:{id:string;name:string}[],inspection=true,dawn=true):GameState{
 ...
 s.players[d]!.presence='dead';s.players[d]!.revealed=true;for(const __discarded of [...s.players[d]!.hand.filter(id=>id!==fusen&&id!==day)])moveToDiscard(s,__discarded,{faceUp:true});s.players[d]!.hand=[];
 if(dawn)s.deck=[fusen,day,...s.deck];else{for(const __discarded of [day,...s.deck])moveToDiscard(s,__discarded,{faceUp:true});s.deck=[fusen];}
