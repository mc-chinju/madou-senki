...
export function makeReclaimWandering(players:{id:string;name:string}[]):GameState{
 ...
 readySetup(()=>s,id=>act(id,{type:'PASS_SETUP'}));act(a,{type:'START_TURN'});act(a,{type:'CHOOSE_DRAW',draw:false}); act(a,{type:'ATTACK',cardInstanceId:sword,targetIds:[b,c],dedicated:true});for(let n=0;n<300;n++){const w=s.windows!.at(-1)!;if(w.kind==='normal-defense')return s;act(w.participants[w.cursor]!,{type:'PASS'},w.kind==='damage'?[3]:Array(100).fill(1));}throw Error('COMBINED_DEATH_WINDOW');
}
