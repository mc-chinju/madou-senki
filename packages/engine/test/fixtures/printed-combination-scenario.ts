...
export function makePrintedCombinationScenario(players:{id:string;name:string}[],counter=false):GameState{
 ...
 const act=(actorId:string,command:import('@madou/protocol').GameCommand)=>{const r=transition(s,{actorId,command},entropy());if(!r.ok)throw Error(`COMBINATION_FIXTURE_${r.code}`);s=r.state;};for(const id of [a,b,c,d])act(id,{type:'PASS_SETUP'});act(a,{type:'START_TURN'});act(a,{type:'CHOOSE_DRAW',draw:false});
