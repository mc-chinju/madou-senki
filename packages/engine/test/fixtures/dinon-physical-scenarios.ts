...
export function makeDinonPhysicalScenario(name:DinonPhysicalScenario,players:{id:string;name:string}[],beforeStart=false){
 ...
 for(const id of [a,b,c,d]){if(zones&&(id===a||id===b))act(id,{type:'PLACE_INITIAL_FOLLOWER',cardInstanceId:id===a?'a2-p18-r3c3':'a2-p21-r2c2'});act(id,{type:'PASS_SETUP'});}if(beforeStart)return s;
