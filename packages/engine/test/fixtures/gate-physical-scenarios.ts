...
export function makeGatePhysicalScenario(name:GatePhysicalScenario,players:{id:string;name:string}[],options:{level?:number;donorSaint?:boolean}={}){
 ...
 for(const p of players){for(const cardInstanceId of p.id===a?own:p.id===b?[front,rear]:[])act(p.id,{type:'PLACE_INITIAL_FOLLOWER',cardInstanceId});act(p.id,{type:'PASS_SETUP'});}readySetup(()=>s,id=>act(id,{type:'PASS_SETUP'}));act(a,{type:'START_TURN'});act(a,{type:'CHOOSE_DRAW',draw:false});return s;
}
