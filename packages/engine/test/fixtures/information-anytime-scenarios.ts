...
export function makeInformationAnytimeScenario(name:'reclaim-peace'|'reclaim-revelation',players:{id:string;name:string}[]):GameState {
 ...
 for(const id of [a,b,c,d]){if(id===b)act(b,{type:'PLACE_INITIAL_FOLLOWER',cardInstanceId:follower});act(id,{type:'PASS_SETUP'});}readySetup(()=>s,id=>act(id,{type:'PASS_SETUP'}));
