...
export function makeR6ExhaustionScenario(players:{id:string;name:string}[]):GameState {
 ...
 const book=takeCard(s,a,'a2-p03-r1c1');trimHand(s,a,book);const ordinary=s.deck.filter(id=>getAction(id)!.category!=='open'),open=s.deck.filter(id=>getAction(id)!.category==='open');s.deck=[ordinary[0]!];s.discard=ordinary.slice(1,3);s.players[b]!.hand.push(...ordinary.slice(3));s.players[c]!.open.push(...open);
 ...
 for(const p of players)act(p.id,{type:'PASS_SETUP'});act(a,{type:'START_TURN'});act(a,{type:'CHOOSE_DRAW',draw:false});act(a,{type:'PLAY_TURN_CARD',cardInstanceId:'a2-p03-r1c1'});for(let n=0;n<100;n++){if(s.windows?.at(-1)?.kind==='after-roll')return s;const w=s.windows!.at(-1)!;act(w.participants[w.cursor]!,{type:'PASS'});}throw Error('S23_FIXTURE_ROLL');
}
