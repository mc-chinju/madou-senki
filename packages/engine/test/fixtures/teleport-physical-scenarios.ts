...
export function makeTeleportPhysicalScenario(name:TeleportPhysicalScenario,players:{id:string;name:string}[],mental=false){
 ...
 for(const p of players)act(p.id,{type:'PASS_SETUP'});act(a,{type:'START_TURN'});act(a,{type:'CHOOSE_DRAW',draw:false});act(a,{type:'ATTACK',cardInstanceId:attack,targetIds:[b],dedicated:false});for(let n=0;n<300;n++){const w=s.windows!.at(-1)!;if(w.kind==='normal-defense')return s;act(w.participants[w.cursor]!,{type:'PASS'});}throw Error('TELEPORT_FIXTURE_WINDOW');
}
