...
it.each([false,true])('Physical Ice Mirror real Prayer changes its relative limit without creating ordinary warrior permission dedicated=%s',dedicated=>{
 ...
 expect(s.actions![id]!.technique).toMatchObject({useLevel:6,effectLevel:8,reflectMagicLimit:8,blockWarriorLimit:dedicated?8:-1});s=finish(s);expect([s.players.A!.damage,s.players.B!.damage]).toEqual([5,0]);expect(s.discard.filter(id=>id===CARD)).toHaveLength(1);
});
