...
it.each(['竜皇子アスフェルト','黒騎士ガーウィン'])('%s ordinary mental defense has no conversion bonus and conceals base and allegiance',name=>{
 ...
 for(const id of ['B','C','D']){const v=viewFor(s,id);expect(v.currentRoll).not.toHaveProperty('threshold');expect(v.currentRoll).not.toHaveProperty('success');expect(v.players.A).not.toHaveProperty('characterId');expect(v.players.A).not.toHaveProperty('faction');expect(v.players.A).not.toHaveProperty('stats');expect(v.players.A).not.toHaveProperty('currentObjective');}
});
