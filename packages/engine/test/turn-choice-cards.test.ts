...
it('Dinon exchanges only both residual hands once after its cancellable source payment',()=>{
 ...
 expect(s.resolution).toContain(SWAP);s=finish(s);expect(s.players.A!.hand).toEqual(b);expect(s.players.B!.hand).toEqual(a);expect(s.discard).toContain(SWAP);expect(s.phase).toBe('hand-adjustment');
...
it('Arseil Farseeing replaces identity inspection with automatic hand inspection and an optional one-card discard',()=>{
 ...
 const card=d.cards[0]!.cardInstanceId;s=act(s,'A',{type:'CHOOSE_INSPECTION',decisionId:d.decisionId,choice:'discard-one',cardInstanceId:card});s=finish(s);expect(s.discard).toContain(card);expect(s.used).toEqual(before);expect(s.phase).toBe('hand-adjustment');
});
...
it('Mother attachment is discarded once when a later real conversion makes Asfelt EVIL',()=>{
 ...
 s=closeWindow(play(s,EVIL));s=closeWindow(s,[6,6]);s=finish(s);expect(s.players.B!.faction).toBe('EVIL');expect(s.players.B!.attachments).not.toContain(MOTHER);expect(s.discard.filter(id=>id===MOTHER)).toHaveLength(1);
});
