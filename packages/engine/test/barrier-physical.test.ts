...
it.each(rows)('%s physical %s actual Fate cancellation keeps payment and original magic damage',(scenario,card)=>{
 let s=makeBarrierPhysicalScenario(scenario,players);s=act(s,'B',{type:'PLAY_DEFENSE',cardInstanceId:card,dedicated:false});const id=Object.values(s.actions!).find(a=>a.cardInstanceId===card)!.id;s=act(s,'C',{type:'PLAY_REACTION',cardInstanceId:'a2-p02-r2c3',mode:'cancel',targetActionId:id});s=finish(s);expect(s.rolls?.filter(r=>r.purpose==='excess-level')??[]).toEqual([]);expect(s.players.B!.damage).toBe(5);expect(discardIds(s).filter(id=>id===card)).toHaveLength(1);expect(s.players.B!.hand).not.toContain(card);
});
...
it.each(rows)('%s physical %s optional decline and actual follower start retain the unspent source',(scenario,card)=>{
 for(const followers of [false,true]){let s=makeBarrierPhysicalScenario(scenario,players);if(followers){s=act(s,'B',{type:'START_FOLLOWERS'});reject(s,'B',{type:'PLAY_DEFENSE',cardInstanceId:card,dedicated:false});}s=finish(s);expect(s.players.B!.damage).toBe(5);expect(s.players.B!.hand).toContain(card);expect(discardIds(s)).not.toContain(card);}
});
