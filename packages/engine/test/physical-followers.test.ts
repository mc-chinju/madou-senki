...
it('real Uonos transformation immediately discards the now-incompatible Gainas castle',()=>{let s=ready();character(s,'A','邪祭ウーノス');const castle=place(s,'A','ガイナス城');handCard(s,'A','復活の儀式');s=act(s,'A',{type:'USE_REVIVAL_RITUAL'});s=closeWindow(s);expect(s.players.A!.faction).toBe('ヴァンミール');expect(s.players.A!.followers).toEqual([]);expect(discardIds(s)).toContain(castle);});
