...
describe('G08 arithmetic helpers (compositions without a current canonical producer)',()=>{
 it.each([
  {base:5,floor:null,add:[1],multipliers:[2,0.5],want:6},
  {base:null,floor:null,add:[1],multipliers:[2,0.5],want:null},
 ...
 ])('S32 abstract resolver composes $base / $floor / $add / $multipliers to $want',({base,floor,add,multipliers,want})=>{expect(composeValue(base,floor,add,multipliers)).toBe(want);});
