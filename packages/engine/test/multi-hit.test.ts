...
it('snapshots a nonzero-HP follower once and subtracts its HP from every simultaneous hit',()=>{
  ...
  expect(s.players.B!.damage).toBe(18); expect(s.players.B!.followers).toEqual([]); expect(discardIds(s)).toContain(soldier);
});
