...
it.each(recoveryCases)('%s declines without auto-recovery',(id,name)=>{let {s,card}=source(id,name);s=finish(pass(s));expect(s.discard).toContain(card);expect(s.players.A!.hand).not.toContain(card);expect(s.players.A!.reclaimUsage).toBeUndefined();});
it.each(recoveryCases)('%s cancellation consumes its selected attempt without a physical clone',(id,name,right)=>{let {s,card}=source(id,name);s=elect(s,right);expect(s.resolution).toContain(card);expect(s.reclaimReservations).not.toContain(card);const f=Object.values(s.abilities!).find(a=>a.context.kind==='reclaim')!;expect(f.abilityId).toBe(id);s=pass(s);const fate=s.players.B!.hand.find(id=>getAction(id)!.name==='命運凶変')!;s=finish(act(s,'B',{type:'PLAY_REACTION',cardInstanceId:fate,mode:'cancel-ability',targetAbilityId:f.id}));expect(s.discard).toContain(card);expect(s.players.A!.hand).not.toContain(card);expect(s.players.A!.reclaimUsage?.[name]?.baseSpent??false).toBe(false);if(right==='extra')expect(s.players.A!.reclaimUsage?.[name]?.extraSpentByAbility).toEqual([id]);expect(s.reclaimDecisions!.find(d=>d.cardInstanceId===card)!.attemptedClaimIds).toHaveLength(1);});
it.each(recoveryCases)('%s saves and resumes one recovery declaration',(id,name,right)=>{let {s,card}=source(id,name);s=elect(s,right);const f=Object.values(s.abilities!).find(a=>a.context.kind==='reclaim')!;expect(f.context).toMatchObject({cardInstanceId:card,sourceCharacterId:id.split('-ab')[0]});expect(viewFor(s,'A').reclaim!.stage).toBe('ability-declaration');s=finish(s);expect(s.players.A!.hand.filter(x=>x===card)).toHaveLength(1);expect(s.discard).not.toContain(card);expect(s.players.A!.reclaimUsage?.[name]?.baseSpent??false).toBe(false);expect(Object.values(s.abilities!)).toEqual([]);});
...
it('Reveal requirement is checked at declaration and resolution without auto-reveal',()=>{let {s,card}=source(rows[1][0],rows[1][1]);s.players.A!.revealed=false;expect(viewFor(s,'A').reclaim!.claims.map(c=>c.right)).toEqual(['base']);s=act(s,'A',{type:'REVEAL_CHARACTER'});s=elect(s,'unlimited');s.players.A!.revealed=false;s=finish(s);expect(s.discard).toContain(card);expect(s.players.A!.revealed).toBe(false);});
...
it.each([
 ...
 s=finish(s);
 expect(s.players.A!.hand).not.toContain(card);expect(s.discard.filter(x=>x===card)).toHaveLength(1);
...
it.each([
 ...
 expect(s.windows??[]).toEqual([]);expect(s.discard.filter(c=>c===card)).toHaveLength(1);
...
it.each([
 ...
 expect(s.reclaimReservations).toContain(card);expect(s.players.A!.hand).not.toContain(card);expect(s.discard).not.toContain(card);
 s=finish(JSON.parse(JSON.stringify(s)));
 expect(s.players.A!.hand.filter(c=>c===card)).toHaveLength(1);expect(s.discard).not.toContain(card);expect(s.players.A!.followers).toEqual([]);
 expect(s.players.A!.reclaimUsage?.[name]?.baseSpent??false).toBe(false);
});
