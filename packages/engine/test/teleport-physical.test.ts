...
it.each(rows)('%s physical %s succeeds for only one real chanted hit and one recipient',(_scenario,card)=>{
 for(const three of [false,true]){let s=makeR6MaaiScenario(players,three,card);s=finish(act(s,'B',{type:'PLAY_DEFENSE',cardInstanceId:card,dedicated:false}));expect(s.players.B!.damage).toBe(three?14:0);expect(s.players.C!.damage).toBe(three?0:7);expect(s.discard.filter(id=>id===card)).toHaveLength(1);}
});
it.each(rows)('%s physical %s actual Fate cancellation pays without generating a teleport check',(scenario,card)=>{
 let s=makeTeleportPhysicalScenario(scenario,players);s=act(s,'B',{type:'PLAY_DEFENSE',cardInstanceId:card,dedicated:false});const defense=Object.values(s.actions!).find(a=>a.cardInstanceId===card)!;s=act(s,'C',{type:'PLAY_REACTION',cardInstanceId:'a2-p02-r2c3',mode:'cancel',targetActionId:defense.id});s=finish(s);expect(s.rolls?.some(r=>r.purpose==='teleport')??false).toBe(false);expect(s.players.B!.damage).toBe(4);expect(s.discard.filter(id=>id===card)).toHaveLength(1);
});
