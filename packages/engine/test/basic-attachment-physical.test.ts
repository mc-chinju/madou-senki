...
it.each(rows)('%s installs exact %s on its user and increases only %s by one after resolution',(scenario,card,stat)=>{
 ...
 expect(s.players.A!.attachments.filter(id=>id===card)).toHaveLength(1);expect(s.discard).not.toContain(card);expect(s.resolution).not.toContain(card);expect(s.phase).toBe('hand-adjustment');
...
it.each(rows)('%s cancellation discards exact %s without adding %s or reopening the action',(scenario,card,stat)=>{
 ...
 expect(s.discard.filter(id=>id===card)).toHaveLength(1);expect(s.players.A!.hand).not.toContain(card);expect(s.phase).toBe('hand-adjustment');
});
...
it.each(rows)('%s actual later opponent Wish takes %s into hand and removes its installed %s bonus',(scenario,card,stat)=>{
 ...
 expect(gameStats(s,'A')).toEqual(before);expect(gameStats(s,'B')).toEqual(recipient);expect(s.discard).not.toContain(card);expect(s.phase).toBe('hand-adjustment');
});
