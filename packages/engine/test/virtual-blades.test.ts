...
it.each(sources)('%s declares its exact virtual profile and finishes without physical source disposal',(name,abilityId,element,damage,maaiRequired)=>{
 let s=setup(name);const hands=Object.fromEntries(s.seatOrder.map(id=>[id,[...s.players[id]!.hand]])),discard=[...s.discard];
 ...
 s=finish(s);expect(s.reclaimDecisions??[]).toEqual([]);expect(s.players.B!.damage).toBe(damage);expect(s.discard).toEqual(discard);for(const id of s.seatOrder)expect(s.players[id]!.hand).toEqual(hands[id]);expect(s.phase).toBe('withdrawal');expect(s.actions).toEqual({});expect(s.groups).toEqual({});
});
it.each(sources)('%s Fate cancellation consumes the attack without creating a virtual discard',(name,abilityId)=>{
 let s=setup(name);const hand=[...s.players.A!.hand],discard=[...s.discard];s=act(s,'A',{type:'DECLARE_VIRTUAL_BLADE',abilityId,targetIds:['B']});while(viewFor(s,'C').activeWindow?.pendingActorId!=='C')s=pass(s);
 s=act(s,'C',{type:'PLAY_REACTION',cardInstanceId:'a2-p02-r2c3',mode:'cancel-ability',targetAbilityId:viewFor(s,'C').reactionTargetAbilityId!});s=finish(s);expect(s.players.B!.damage).toBe(0);expect(s.players.A!.hand).toEqual(hand);expect(s.discard).toEqual([...discard,'a2-p02-r2c3']);expect(s.phase).toBe('withdrawal');expect(s.actions).toEqual({});
});
...
it.each(sources)('%s resumes a real physical reflection without inventing an original card',(name,abilityId,element,damage)=>{
 ...
 expect(viewFor(s,'A').currentAttack!.attackerId).toBe('B');const child=Object.values(s.actions!).find(a=>a.actorId==='B')!;expect(child).toMatchObject({fixedReceivedEffect:true,effectSourceAbilityId:abilityId,sourceCardInstanceIds:[]});s=finish(s);expect(s.players.B!.damage).toBe(0);expect(s.players.A!.damage).toBe(damage);expect(s.discard.filter(id=>id===mirror)).toHaveLength(1);expect(s.phase).toBe('withdrawal');
});
