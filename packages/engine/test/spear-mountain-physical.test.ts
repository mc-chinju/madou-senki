...
it.each(rows)('%s physical %s selected=%s actual Fate cancellation preserves incoming hit and offers a different defense',(scenario,card,dedicated)=>{
 let s=act(makeSpearMountainScenario(scenario,players),'B',{type:'PLAY_DEFENSE',cardInstanceId:card,dedicated});const id=Object.values(s.actions!).find(a=>a.cardInstanceId===card)!.id;while(s.windows!.at(-1)!.participants[s.windows!.at(-1)!.cursor]!=='C')s=pass(s);s=act(s,'C',{type:'PLAY_REACTION',cardInstanceId:'a2-p02-r2c3',mode:'cancel',targetActionId:id});s=until(s,'normal-defense');reject(s,'B',{type:'PLAY_DEFENSE',cardInstanceId:card,dedicated:false});expect(discardIds(s).filter(x=>x===card)).toHaveLength(1);s=finish(act(s,'B',{type:'PLAY_DEFENSE',cardInstanceId:'a2-p05-r3c1',dedicated:false}));expect([s.players.A!.damage,s.players.B!.damage]).toEqual([0,0]);
});
