...
it('keeps turn recovery forced failure after another player rerolls, without refill or replay',()=>{
 let s=ready();s.phase='turn-start';s.players.A!.statuses=[{id:'stopped',kind:'stopped',modifiers:[0],nextCheck:1}];s.discard.push(...s.players.A!.hand.splice(2));const fate=handCard(s,'B','命運凶変');const god=handCard(s,'C','神性介入');s=act(s,'A',{type:'START_TURN'});s=closeAndDeclineDispositions(s,[1,1]);const id=publicRoll(s).rollId;s=pass(s);s=act(s,'B',{type:'PLAY_REACTION',cardInstanceId:fate,mode:'force-fail',targetRollId:id});s=closeAndDeclineDispositions(s);s=pass(s);s=pass(s);s=act(s,'C',{type:'PLAY_REACTION',cardInstanceId:god,mode:'reroll',targetRollId:id});s=closeAndDeclineDispositions(s,[2,2]);s=closeAndDeclineDispositions(s);expect(s.players.A!.statuses).toEqual([{id:'stopped',kind:'stopped',modifiers:[0],nextCheck:2}]);expect(s.players.A!.hand).toHaveLength(2);expect(s.turnSeat).toBe(1);expect(s.rolls!.at(-1)).toMatchObject({id,faces:[2,2],success:false,forcedFailure:true,stage:'applied'});
});
