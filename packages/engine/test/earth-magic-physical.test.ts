...
it.each(rows)('%s actual Evade defends only B without changing dedicated C damage',scenario=>{
 const m=earthMagicMode(scenario);let s=until(act(makeEarthMagicScenario(scenario,players),'A',command(scenario)),'normal-defense');s=finish(act(s,'B',{type:'PLAY_DEFENSE',cardInstanceId:'a2-p05-r3c1',dedicated:false}));expect([s.players.B!.damage,s.players.C!.damage]).toEqual([0,m.dedicated?m.damage:0]);expect(discardIds(s)).toContain('a2-p05-r3c1');
});
it.each(rows)('%s actual Fate cancels then base reclaim and later real turn reuse preserve elected values',scenario=>{
 const m=earthMagicMode(scenario);let s=act(makeEarthMagicScenario(scenario,players),'A',command(scenario));const id=Object.values(s.actions!).find(a=>a.cardInstanceId===m.card)!.id;s=pass(s);s=pass(s);s=act(s,'C',{type:'PLAY_REACTION',cardInstanceId:'a2-p02-r2c3',mode:'cancel',targetActionId:id});s=finish(claim(s,m.card));expect([s.players.B!.damage,s.players.C!.damage]).toEqual([0,0]);expect(s.players.A!.hand.filter(id=>id===m.card)).toHaveLength(1);expect(s.players.A!.reclaimUsage?.[m.name]?.baseSpent).toBe(true);s=nextOwn(s,m.card);s=finish(act(s,'A',command(scenario)));expect([s.players.B!.damage,s.players.C!.damage]).toEqual([m.damage,m.dedicated?m.damage:0]);expect(discardIds(s).filter(id=>id===m.card)).toHaveLength(1);
});
