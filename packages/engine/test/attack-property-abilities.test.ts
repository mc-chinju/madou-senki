...
describe('Task7q real canonical attack property packages',()=>{
 ...
 it('actual Lancaster dedicated SkySpear shared advance affects B/C independently',()=>{
  ...
  s=passReclaims(act(s,'B',{type:'PLAY_MAAI',cardInstanceId:b[2]}));s=passReclaims(act(s,'C',{type:'PLAY_MAAI',cardInstanceId:c[2]}));s=pass(s);expect([s.players.B!.damage,s.players.C!.damage]).toEqual([0,0]);expect(s.distances).toEqual(distances);expect(s.discard.filter(id=>id===advance)).toHaveLength(1);
 });
