...
describe('Task7q real canonical attack property packages',()=>{
 ...
 it('actual BlackWing native two plus Wind needs third; shared advance preserves paid progress/reload',()=>{
  ...
  check(0,0,0,3);s=act(s,'B',{type:'PLAY_MAAI',cardInstanceId:cards[0]});check(0,1,1,2);s=act(s,'B',{type:'PLAY_MAAI',cardInstanceId:cards[1]});check(0,2,2,1);expect(s.windows!.at(-1)!.kind).toBe('normal-defense');s=act(s,'B',{type:'PLAY_MAAI',cardInstanceId:cards[2]});check(0,3,3,0);expect(s.windows!.at(-1)!.kind).toBe('defense-advance');s=act(s,'A',{type:'PLAY_ADVANCE',cardInstanceId:advance});check(0,3,2,1,1);s=pass(s);check(2,0,2,1);s=act(s,'B',{type:'PLAY_MAAI',cardInstanceId:cards[3]});check(2,1,3,0);s=pass(s);expect(viewFor(s,'B').maaiDefense).toBeNull();expect(s.players.B!.damage).toBe(0);expect(s.distances).toEqual(distances);for(const id of [...cards,advance])expect(s.discard.filter(c=>c===id)).toHaveLength(1);
 });
 it('actual Lancaster dedicated SkySpear shared advance affects B/C independently',()=>{
  ...
  for(const [actor,cards] of [['B',b],['C',c]] as const)for(const card of cards.slice(0,2))s=act(s,actor,{type:'PLAY_MAAI',cardInstanceId:card});
  s=act(s,'A',{type:'PLAY_ADVANCE',cardInstanceId:advance});expect(viewFor(s,'C').maaiDefense).toMatchObject({sharedAdvances:1,responding:true,targets:[{actorId:'B',required:2,submitted:2,effective:1,remaining:1},{actorId:'C',required:2,submitted:2,effective:1,remaining:1}]});s=pass(s);
  s=act(s,'B',{type:'PLAY_MAAI',cardInstanceId:b[2]});s=act(s,'C',{type:'PLAY_MAAI',cardInstanceId:c[2]});s=pass(s);expect([s.players.B!.damage,s.players.C!.damage]).toEqual([0,0]);expect(s.distances).toEqual(distances);expect(s.discard.filter(id=>id===advance)).toHaveLength(1);
 });
...
it('actual ordinary Heavenly Hundred Slash Wind resets submitted/carried progress for each hit',()=>{
 ...
 for(let i=0;i<2;i++){expect(viewFor(s,'B').maaiDefense).toMatchObject({hitIndex:i,targets:[{hitIndex:i,carried:0,submitted:0,required:2,remaining:2}]});for(const card of cards.slice(i*2,i*2+2))s=act(s,'B',{type:'PLAY_MAAI',cardInstanceId:card});s=pass(s);if(i===0)expect(s.groups![g.id]!.targets[0]!.hits.map(h=>h.defended)).toEqual([true,false]);}expect(s.players.B!.damage).toBe(0);expect(viewFor(s,'B').maaiDefense).toBeNull();
});
it('helper source suppression between partial maai submissions uses current native requirement without refund',()=>{
 let s=until(selected(),'normal-defense');const cards=maais(s,'B',2);s=act(s,'B',{type:'PLAY_MAAI',cardInstanceId:cards[0]});s.players.A!.statuses=[{id:'helper-partial-seal',kind:'ability-disabled',modifiers:[0],nextCheck:0}];expect(viewFor(s,'B').maaiDefense!.targets[0]).toMatchObject({required:2,submitted:1,remaining:1});s=act(s,'B',{type:'PLAY_MAAI',cardInstanceId:cards[1]});expect(s.windows!.at(-1)!.kind).toBe('defense-advance');s=pass(s);expect(s.players.B!.damage).toBe(0);for(const card of cards)expect(s.discard.filter(c=>c===card)).toHaveLength(1);
});
it('actual SkySpear completed maai for B survives C Shadow→Confusion while later D needs native one',()=>{
 let s=selected(WIND,'竜殺天空槍',['B','C','D'],true);character(s,'C','忍びのイダ');s.players.A!.permanent={endurance:50};const seal=handCard(s,'C','錯乱');s=until(s,'normal-defense');const g=Object.values(s.groups!)[0]!,cards=maais(s,'B',2);for(const card of cards)s=act(s,'B',{type:'PLAY_MAAI',cardInstanceId:card});s=shadowChild(s,'C',seal,g.id,'D');expect(viewFor(s,'D').maaiDefense).toMatchObject({targetId:'D',targets:[{actorId:'B',required:1,submitted:2,effective:2,remaining:0},{actorId:'C',closed:true},{actorId:'D',required:1,remaining:1}]});const [d]=maais(s,'D',1);s=act(s,'D',{type:'PLAY_MAAI',cardInstanceId:d});s=pass(s);expect([s.players.B!.damage,s.players.D!.damage]).toEqual([0,0]);for(const card of cards)expect(s.discard.filter(c=>c===card)).toHaveLength(1);
});
