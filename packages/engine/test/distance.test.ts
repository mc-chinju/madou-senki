...
it('scopes shared maai to one hit and opens fresh defenses for the next simultaneous hit',()=>{let s=ready();character(s,'A','侍大将のシン');const attack=handCard(s,'A','天地百撃斬');const [bMaai,cMaai]=handCards(s,['B','C'],'間合い／休息');s.players.A!.hand=s.players.A!.hand.filter(id=>id!==attack);s.players.A!.chants.push({cardInstanceId:attack,revealed:false});s=act(s,'A',{type:'ATTACK',cardInstanceId:attack,targetIds:['B','C'],dedicated:true});s=until(s,'damage');s=closeWindow(s,[2]);s=closeWindow(s);s=until(s,'normal-defense');s=passReclaims(act(s,'B',{type:'PLAY_MAAI',cardInstanceId:bMaai}));s=passReclaims(act(s,'C',{type:'PLAY_MAAI',cardInstanceId:cMaai}));s=act(s,'A',{type:'PASS'});expect(s.windows!.at(-1)!.continuation).toMatchObject({targetId:'B'});s=finish(s);expect(s.players.B!.damage).toBe(7);expect(s.players.C!.damage).toBe(7);});
