...
it('Physical parry actual Prayer raises only its effect level and never creates a returned attack',()=>{
 ...
 expect(s.actions![id]!.technique).toMatchObject({useLevel:3,effectLevel:9,damage:null});expect(s.rolls!.filter(r=>r.purpose==='excess-level')).toHaveLength(checks);s=finish(s);expect([s.players.A!.damage,s.players.B!.damage]).toEqual([0,0]);expect(s.groups).toEqual({});expect(discardIds(s).filter(id=>id===CARD)).toHaveLength(1);expect(discardIds(s)).toContain('a2-p05-r2c3');
});
it('Physical parry cancels only one hit from an actually chanted three-hit warrior attack',()=>{
 let s=makeR6MaaiScenario(players,true,CARD);s=finish(declare(s));expect(s.players.B!.damage).toBe(14);expect(s.players.A!.damage).toBe(0);expect(discardIds(s).filter(id=>id===CARD)).toHaveLength(1);
});
it('Physical parry can be declined or canceled by an actual Fate while the original hit survives',()=>{
 for(const canceled of [false,true]){let {s,attack}=source();s=until(act(s,'A',{type:'ATTACK',cardInstanceId:attack,targetIds:['B'],dedicated:false}),'normal-defense');if(canceled){s=declare(s);const defense=Object.values(s.actions!).find(a=>a.cardInstanceId===CARD)!;s=act(s,'C',{type:'PLAY_REACTION',cardInstanceId:'a2-p02-r2c3',mode:'cancel',targetActionId:defense.id});}s=finish(s);expect(s.players.B!.damage).toBe(4);expect(s.players.B!.hand.includes(CARD)).toBe(!canceled);expect(discardIds(s).filter(id=>id===CARD)).toHaveLength(canceled?1:0);}
});
