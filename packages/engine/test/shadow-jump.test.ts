...
it.each(['cost','attack','child'] as const)('Shadow jump self-check has no enemy check and defense survives declining %s',choice=>{
 ...
 if(choice!=='cost'){s=act(s,'B',{type:'PAY_SHADOW_JUMP',abilityEventId:f.id,advanceCardInstanceId:advance});s=until(s,'ability-attack');expect(s.discard.filter(id=>id===advance)).toHaveLength(1);expect(s.players.B!.hand.length).toBe(paidBefore.length-1);if(choice==='child'){s=until(act(s,'B',{type:'ATTACK',cardInstanceId:child,targetIds:['A'],dedicated:false}),'normal-defense');expect(Object.values(s.actions!).find(a=>a.actorId==='B')!.technique).toMatchObject({followerIgnore:true,maaiProhibited:true});}}
...
it('Shadow jump cost rejects stale, foreign and non-advance payment before mutation; repeated payment is refused',()=>{
 ...
 s=until(act(s,'B',{type:'PAY_SHADOW_JUMP',abilityEventId:id,advanceCardInstanceId:advance}),'ability-attack');const before=JSON.stringify(s);expect(transition(s,{actorId:'B',command:{type:'PAY_SHADOW_JUMP',abilityEventId:id,advanceCardInstanceId:advance}},entropy()).ok).toBe(false);expect(JSON.stringify(s)).toBe(before);s=finish(s);expect(s.discard.filter(x=>x===advance)).toHaveLength(1);expect(s.players.B!.damage).toBe(0);
});
...
it('Actual initial follower cannot absorb the shadow child and maai is rejected without payment',()=>{
 ...
 for(const actor of s.seatOrder){if(actor==='A')s=act(s,actor,{type:'PLACE_INITIAL_FOLLOWER',cardInstanceId:soldier});s=act(s,actor,{type:'PASS_SETUP'});}s=act(s,'A',{type:'START_TURN'});s=act(s,'A',{type:'CHOOSE_DRAW',draw:false});s=until(act(s,'A',{type:'ATTACK',cardInstanceId:source,targetIds:['B'],dedicated:false}),'normal-defense');s=until(use(s),'shadow-jump-cost');const followers=structuredClone(s.players.A!.followers);s=until(act(s,'B',{type:'PAY_SHADOW_JUMP',abilityEventId:Object.values(s.abilities!)[0]!.id,advanceCardInstanceId:advance}),'ability-attack');s=until(act(s,'B',{type:'ATTACK',cardInstanceId:child,targetIds:['A'],dedicated:false}),'normal-defense');const childAction=Object.values(s.actions!).find(a=>a.actorId==='B')!;
 expect(s.rolls!.filter(r=>r.resume.kind==='action-check'&&r.resume.actionId===childAction.id).length).toBeGreaterThan(0);expect(viewFor(s,'A').legalChoices).not.toContain('PLAY_MAAI');const before=JSON.stringify(s);expect(transition(s,{actorId:'A',command:{type:'PLAY_MAAI',cardInstanceId:maai}},entropy()).ok).toBe(false);expect(JSON.stringify(s)).toBe(before);s=finish(s);expect([s.players.A!.damage,s.players.B!.damage]).toEqual([6,0]);expect(s.players.A!.followers).toEqual(followers);expect(s.players.A!.hand).toContain(maai);expect(s.discard).not.toContain(soldier);
});
