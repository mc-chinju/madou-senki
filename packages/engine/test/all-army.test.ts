...
it('actual Griffon pays both cards, declares parent then follower, checks morale and resolves two ordinary hits',()=>{
 ...
 s=closeWindow(s);expect(viewFor(s,'A').currentAction).toMatchObject({cardInstanceId:command.followerCardInstanceId});s=closeWindow(s);expect(s.rolls!.at(-1)!.purpose).toBe('follower-morale');s=closeWindow(s,[1,1]);expect(s.rolls!.at(-1)!.threshold).toBe(gameStats(s,'A').spirit);s=finish(s);expect(s.players.B!.damage).toBe(16);expect(s.discard).toEqual(expect.arrayContaining([ARMY,command.followerCardInstanceId]));expect(s.phase).toBe('withdrawal');expect(Object.keys(s.actions!)).toEqual([]);
});
it.each(['parent','follower','morale'])('Fate at %s cancels only its accepted declaration/check and consumes both cards without an attack',stage=>{
 ...
 s=act(s,'B',stage==='morale'?{type:'PLAY_REACTION',cardInstanceId:fate,mode:'force-fail',targetRollId:viewFor(s,'B').reactionTargetRollId!}:{type:'PLAY_REACTION',cardInstanceId:fate,mode:'cancel',targetActionId:viewFor(s,'B').reactionTargetActionId!});s=finish(s);expect(s.players.B!.damage).toBe(0);expect(s.discard).toEqual(expect.arrayContaining([ARMY,command.followerCardInstanceId]));expect(s.players.A!.hand).not.toContain(command.followerCardInstanceId);expect(s.phase).toBe('withdrawal');expect(s.reclaimDecisions!.filter(d=>d.cardInstanceId===command.followerCardInstanceId).every(d=>d.source.kind==='ordinary-disposition'&&d.source.trigger==='named-card-used')).toBe(true);
});
it('failed morale cannot reselect, preserves a frozen result and creates no follower-death recovery',()=>{
 let {s,command}=prepared();s.players.A!.permanent={endurance:100,spirit:-20};s=closeWindow(closeWindow(act(s,'A',command)));s=closeWindow(s,[1,1]);expect(s.rolls!.at(-1)!.success).toBe(false);const roll=structuredClone(s.rolls!.at(-1)!);s=finish(s);expect(s.players.B!.damage).toBe(0);expect(s.rolls!.find(r=>r.id===roll.id)!.threshold).toBe(0);reject(s,'A',command);expect(s.discard.filter(id=>id===ARMY)).toHaveLength(1);
});
