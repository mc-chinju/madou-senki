...
it.each(rows)('%s actual Fate cancellation pays its source without damage or extra effects',scenario=>{
 const m=mountainBreakerMode(scenario);let s=act(makeMountainBreakerScenario(scenario,players),'A',command(scenario));const id=Object.values(s.actions!).find(a=>a.cardInstanceId===m.card)!.id;s=pass(s);s=pass(s);s=act(s,'C',{type:'PLAY_REACTION',cardInstanceId:'a2-p02-r2c3',mode:'cancel',targetActionId:id});s=finish(s);expect(s.players.B!.damage).toBe(0);expect(discardIds(s).filter(id=>id===m.card)).toHaveLength(1);expect(s.phase).toBe('withdrawal');
});
