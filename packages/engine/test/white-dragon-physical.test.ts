...
it.each([['white-sword-ordinary','小悪魔'],['white-sword-ordinary','スケルトン'],['white-slash-ordinary','小悪魔'],['white-slash-ordinary','スケルトン']] as const)('%s destroys actual initial %s by black or dead attribute before follower HP', (scenario,follower)=>{
 let s=makeWhiteDragonScenario(scenario,players,{follower});const guard=s.players.B!.followers[0]!.cardInstanceId;s=until(act(s,'A',command(scenario)),'follower-start');s=closeWindow(s);expect(Object.values(s.groups!)[0]!.targets[0]!.followerDefense![0]!.hits[0]!.outcome).toBe('attribute-destroyed');s=finish(s);expect(s.players.B!.damage).toBe(whiteDragonMode(scenario).damage);expect(discardIds(s).filter(id=>id===guard)).toHaveLength(1);
});
