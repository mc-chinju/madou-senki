...
export interface DamageIntent {rewardSnapshot?:import('../abilities/combat-reward-state.js').CombatDamageSnapshot;sadLoveSource?:import('../abilities/sad-love-state.js').SadLoveSource;targetId:PlayerId;damage:number;instantDeath?:boolean;cause:'attack'|'self-damage'|'instant-death'|'maximum-endurance'|'petrification';sourceActorId?:PlayerId;sourceCardInstanceId?:string;sourceAbilityId?:string;groupId?:string;actionId?:string;eventId:string}
export type LifecycleTask = {rootEventIds?:string[]} & (
 | import('../abilities/combat-reward-state.js').CombatRewardTask
 ...
 | {kind:'wish-complete';id:string;decisionId:string;waiting?:boolean}
 | {kind:'turn-card-resume';id:string;actionId:string}
 | {kind:'draw';id:string;actorId:PlayerId;target:number}
 | {kind:'resume-phase';id:string;phase:GameState['phase'];turnSeat?:number}
 | {kind:'protection';id:string;includeNewDefeats?:boolean}
 ...
 | {kind:'death-batch';chamGifts?:Record<string,import('../abilities/cham-death-gift.js').ChamGiftSelection>;rewardBatchId?:string;id:string;actorIds:PlayerId[];cursor:number;intents:DamageIntent[];waiting?:boolean}
...
export interface LifecycleDecision {chamGift?:import('../abilities/cham-death-gift.js').ChamGiftOption;kind:'death-gift'|'revival'|'lifecycle-boundary'|'re-setup';actorId:PlayerId;eligibleTargetIds:PlayerId[];sourceCardInstanceId?:string}
