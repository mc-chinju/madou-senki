...
function live(s:GameState,m:SelectedActionModifier):boolean {const p=s.players[m.actorId];return !!p&&isActive(p)&&canUseCharacterAbility(p)&&ownsAbility(p,m.abilityId);}
...
export function resolveActionModifier(s:GameState,f:AbilityFrame,dice:()=>number):boolean {
 ...
 if((kind==='spirit'||kind==='power')&&f.stage==='declaration'){
  f.stage=kind==='spirit'?'self-check':'numeric';
  f.rollIds.push(beginRoll(s,{eventId:f.eventId,rollerId:f.actorId,purpose:kind==='spirit'?'ability-check':'ability-value',formula:kind==='spirit'?'2d6':'d6',...(kind==='spirit'?{check:{modifier:0}}:{}),resume:{kind:'ability',abilityId:f.id}},dice).id);
  return false;
 }
 const roll=s.rolls?.find(r=>r.id===f.rollIds.at(-1));
 if(kind==='spirit'&&!roll?.success)return true;
 acceptActionModifiers(s,a).selected.push({abilityId:f.abilityId,actorId:f.actorId,...(kind==='power'?{amount:roll!.total!}:kind==='spirit'||kind==='staff'||kind==='black-bow'?{amount:1}:{})});
...
export function effectPreview(s:GameState,a:ActionFrame,targetId:string|undefined=a.targetIds.length===1?a.targetIds[0]:undefined):number {
 ...
 return composeValue(declaration.replacement??m.effectBase,floor,[m.prayerAddition,declaration.effectAddition,conditionalTechniqueAdditions(s,a,targetId).effect,...selected.filter(x=>['staff','spirit','power','black-bow'].includes(VALUE_ABILITIES[x.abilityId])).map(x=>x.amount??0)])!;
}
...
/** Fist's separate numeric roll belongs to this action and starts only after native values. */
export function prepareModifierDamage(s:GameState,a:ActionFrame,dice:()=>number):boolean {
 const m=a.modifiers;if(!m)return true;
 const fist=m.selected.find(x=>VALUE_ABILITIES[x.abilityId]==='fist');
 if(!fist||fist.damageSkipped)return true;
 if(!fist.damageRollId){
  if(!live(s,fist)){fist.damageSkipped=true;return true;}
  fist.damageRollId=beginRoll(s,{eventId:a.eventId,rollerId:a.actorId,purpose:'ability-value',formula:'d6',resume:{kind:'action-value',actionId:a.id,value:'ability-damage'}},dice).id;return false;
 }
 return s.rolls?.find(r=>r.id===fist.damageRollId)?.stage==='applied';
}
export function damagePreview(s:GameState,a:ActionFrame,targetId:string|undefined=a.targetIds.length===1?a.targetIds[0]:undefined):number|null {
 ...
 const selected=m?.selected.filter(x=>live(s,x))??[];
 const fist=selected.find(x=>VALUE_ABILITIES[x.abilityId]==='fist');
 const roll=fist&&!fist.damageSkipped?s.rolls?.find(r=>r.id===fist.damageRollId):undefined;
 const declaration=declarationNumbers(s,a);
 return composeValue(m?.damageBase!==undefined?m.damageBase:a.technique.damage,null,[conditionalTechniqueAdditions(s,a,targetId).damage,declaration.damageAddition,a.technique.damageAdditive??0,selected.some(x=>VALUE_ABILITIES[x.abilityId]==='black-bow')?2:0,roll?.stage==='applied'?roll.total??0:0],
  [declaration.damageMultiplier,a.technique.damageMultiplier??1,a.fromChant?a.technique.chantDamageMultiplier??1:1,selected.some(x=>VALUE_ABILITIES[x.abilityId]==='axe')?2:1,m?.printedDamageDouble?2:1]);
}
