...
export function bindPeaceAction(s:GameState,before:GameState,input:GameInput):void {
 ...
 if(!p||before.phase!=='action'||before.seatOrder[before.turnSeat]!==p.id||!['ATTACK','PLAY_ALL_ARMY','PLAY_TURN_TECHNIQUE','PLAY_TURN_CARD','REST','CHANT','ARRANGE_FOLLOWERS','PASS_ACTION','USE_ABILITY'].includes(c.type)||c.type==='PLAY_TURN_CARD'&&'cardInstanceId' in c&&c.cardInstanceId==='a2-p03-r1c1')return;
