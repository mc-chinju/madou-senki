...
interface Package {half?:boolean;check?:number;dieReduction?:boolean;warriorThreshold?:boolean;reflection?:boolean;threshold?:number;lowDamage?:boolean;reduction?:'magic'|'black';elements?:boolean;warriorOnly?:boolean}
...
function applicable(p:Package,t:Technique):boolean {return !(p.warriorOnly&&t.school!=='warrior'||p.elements&&!t.attributes.some(a=>a==='炎'||a==='水')||(p.reduction==='magic'||p.half)&&t.school!=='magic'||p.reflection&&!!t.counterProhibited);}
...
export function resolveReceivedDefense(s:GameState,f:AbilityFrame,dice:()=>number):boolean {
 ...
 if(p.threshold!==undefined||p.elements||p.lowDamage)received.reserved.push(f.abilityId);
...
export function evaluateReceivedReservations(s:GameState,g:AttackGroup,t:AttackTarget,h:AttackTarget['hits'][number]):void {
 ...
 for(const id of h.receivedDefense?.reserved??[]){const p:Package=PACKAGES[id];if(!activeAbilitySource(s,t.actorId,id)||!applicable(p,technique))continue;
  if(p.warriorThreshold&&technique.effectLevel<=gameStats(s,t.actorId).warrior_level||p.threshold!==undefined&&technique.effectLevel<=p.threshold||p.lowDamage&&(h.damage===null||h.damage<=5)||p.elements&&technique.attributes.some(a=>a==='炎'||a==='水')){h.defended=true;return;}
 }
}
