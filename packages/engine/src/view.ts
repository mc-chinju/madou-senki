...
export interface PlayerView {
  ...
  maaiDefense:MaaiDefenseView|null;
  ...
  pending: { kind: 'initial-followers'; actorId: PlayerId } | null;
  deckCount: number; discard: string[]; distances: GameState['distances']; distanceMarkers: { a:PlayerId;b:PlayerId;ownerId:PlayerId;cardInstanceId:string }[]; players: Record<PlayerId, PublicPlayerView>;
  ...
  activeWindow: { windowId:string; windowRevision:number; kind:string; pendingActorId:PlayerId; reason:string } | null;
  actionCalculation:null|{actionId:string;actorId:string;cardInstanceId:string;effectLevel:number;damage:number|null;calculation:CalculationReadiness};
  currentAction: null|{source:'ability';actionId:string;kind:'ability';actorId:PlayerId;targetIds:PlayerId[];stage:string;label:string;abilityId?:string;abilityName?:string;abilityEffectIds?:import('./abilities/frames.js').AbilityEffectId[]}|{ source:'card'|'follower';sourceZone?:'hand'|'followers'|'chant';coSourceZone?:'hand'|'followers'|'chant';coSourceCardInstanceId?:string;actionId:string;kind:string;actorId:PlayerId;cardInstanceId:string;targetIds:PlayerId[];stage:string;technique:{calculation?:CalculationReadiness;school?:string;range:string;attributes:string[];useLevel:number;effectLevel?:number;damage?:number|null;hitCount?:number} };
  currentAttack: null|{reflection?:{source:'ability';actorId:string;sourceCardInstanceId:string};groupId:string;actionId:string;attackerId:PlayerId;targetIds:PlayerId[];hitIndex:number;targetId:PlayerId|null;reason:string;technique:{effectLevel:number;damage:number|null;attributes:string[];destructionEffects:string[];beastIgnore:boolean};defenseRestrictions:{maaiProhibited:boolean;evadeProhibited:boolean;counterProhibited:boolean;limitedDefenses?:('teleport'|'counter')[]};targets:{actorId:PlayerId;hits:{bodyDamage?:{directDamage:number|null;resistanceDamage:number;total:number};index:number;defended:boolean;hit:boolean;sourceCardInstanceId?:string;technique?:{effectLevel:number;damage:number|null;attributes:string[];destructionEffects:string[];beastIgnore:boolean};damageRollId?:string}[]}[]};
...
function logView(event: GameEvent): LogView {
  ...
  if((event.type==='CARD_GIFTED'||event.type==='BEAST_CAPTURED')&&event.targetId)result.targetId=event.targetId;
  ...
  if (event.type === 'OPEN' || event.type === 'CARD_DRAWN' || (event.type==='CARD_GIFTED'||event.type==='BEAST_CAPTURED')&&event.audience!=='public') result.cardInstanceId = event.cardInstanceId!;
  if (event.type === 'CHARACTER_REVEALED' || event.type === 'CHARACTER_ASSIGNED' || event.type==='CHARACTER_TRANSFORMED' || event.type==='PLAYER_REVIVED') result.characterId = event.characterId!;
  return result;
}
...
export function viewFor(state: GameState, viewerId: PlayerId): PlayerView {
  ...
  const techniqueDecision:PlayerView['techniqueDecision']=hasPriority&&active?.kind==='technique-double-choice'&&activeContinuation?{kind:'damage-double',actionId:activeContinuation.id,actorId:viewerId,sourceCardInstanceId:activeContinuation.cardInstanceId}:hasPriority&&active?.kind==='hit-advance-choice'&&activeGroup?{kind:'hit-advance',groupId:activeGroup.id,actorId:viewerId,sourceCardInstanceId:state.actions![activeGroup.actionId]!.cardInstanceId,cardInstanceIds:advanceCards(state,viewerId)}:null;
  if(techniqueDecision)legalChoices=[techniqueDecision.kind==='damage-double'?'CHOOSE_DAMAGE_DOUBLE':'PAY_HIT_ADVANCES','PASS'];
  const additionalAttack:PlayerView['additionalAttack']=active?.kind==='ability-attack'?activeContinuation?.printedGrant?{source:'card',actorId:activeContinuation.actorId,targetId:activeContinuation.printedGrant.targetId,sourceCardInstanceId:activeContinuation.cardInstanceId}:activeAbility?{source:'ability',actorId:activeAbility.actorId,targetId:activeAbility.targetIds[0]!}:null:null;
  const lifetimeDecision:LifetimeDecision|null=hasPriority&&active!.kind==='lifetime-effect-choice'&&activeGroup&&active!.continuation.kind==='group'?{kind:lifetimeDecisionKind(activeGroup),actorId:viewerId,targetId:active!.continuation.targetId!,sourceCardInstanceId:state.actions![activeGroup.actionId]!.cardInstanceId,choices:['apply','decline']}:null;
  ...
  if(lifecycleTask&&lifecycleActor&&['death-gift','revival','re-setup','lifecycle-boundary'].includes(active!.kind)){lifecycleDecision={kind:active!.kind as LifecycleDecision['kind'],actorId:lifecycleActor,eligibleTargetIds:lifecycleTask.kind==='death-batch'?eligibleGiftRecipients(state,lifecycleTask.id,lifecycleActor):state.seatOrder.filter(id=>id!==lifecycleActor&&isActive(state.players[id]!)),...(lifecycleTask.kind==='fusen'?{sourceCardInstanceId:lifecycleTask.sourceCardInstanceId}:{})};if(hasPriority){if(active!.kind==='revival')legalChoices=['CHOOSE_REVIVAL','PASS'];else if(active!.kind==='re-setup')legalChoices=['PLACE_INITIAL_FOLLOWER','PASS_SETUP'];else if(active!.kind==='death-gift')legalChoices=['PASS',...(self.hand.some(id=>id===(self.faction==='EVIL'?'a2-p02-r3c2':self.faction==='GOOD'?'a2-p02-r3c3':''))?['PLAY_DEATH_GIFT']:[])];}}
  const beastCapture=beastCaptureView(state,viewerId);if(beastCapture)legalChoices=['CHOOSE_BEAST_CAPTURE','PASS'];
  const conditionalAbilities=conditionalAbilitySettings(state,viewerId);if(conditionalAbilities.some(o=>o.canActivate||o.canDeactivate))legalChoices.push('SET_CONDITIONAL_ABILITY');
  const abilityOptions=availableAbilities(state,viewerId);if(abilityOptions.length)legalChoices.push('USE_ABILITY');
  if(hasPriority&&active?.kind==='ability-attack'&&!stopped)legalChoices.push('ATTACK');
  const lifecycleAbilities=availableLifecycleAbilities(state,viewerId);if(lifecycleAbilities.length)legalChoices.push('USE_LIFECYCLE_ABILITY');
  ...
  const actionCalculation:PlayerView['actionCalculation']=calculationSource?{actionId:calculationSource.id,actorId:calculationSource.actorId,cardInstanceId:calculationSource.cardInstanceId,effectLevel:effectPreview(state,calculationSource),damage:damagePreview(state,calculationSource),calculation:calculationReadiness(calculationSource)}:null;
  ...
  }if(projectedAction.stage==='resolve'&&typeof projectedAction.technique.hitCount==='number')technique.hitCount=projectedAction.technique.hitCount;currentAction={...(projectedAction.sourceZone?{sourceZone:projectedAction.sourceZone}:{}),...(projectedAction.coSource?{coSourceZone:projectedAction.coSource.fromFollowers?'followers':projectedAction.coSource.fromChant?'chant':'hand'}:{}),source:projectedAction.followerOrigin?'follower':'card',...(projectedAction.coSource?{coSourceCardInstanceId:projectedAction.coSource.cardInstanceId}:{}),actionId:projectedAction.id,kind:projectedAction.kind,actorId:projectedAction.actorId,cardInstanceId:projectedAction.cardInstanceId,targetIds:[...projectedAction.targetIds],stage:projectedAction.stage,technique};}
  if(currentAction&&activeGroup&&projectedAction?.id===(currentSourceHit?.sourceActionId??activeGroup.actionId)){const current=activeTechnique!;currentAction.technique.school=current.school;currentAction.technique.attributes=[...current.attributes];}
  ...
  return { conditionalAbilities,inspection:inspectionView(state,viewerId),spiritExpiry:spiritExpiryView(self),drawAbilityOptions:drawAbilityOptions(state,viewerId),revealAbilityOptions:revealAbilityOptions(state,viewerId),declarationCandidates:declarationCandidates(state,viewerId),declarationSelection:declarationSelection(state,viewerId),maaiDefense:maaiDefenseView(state,effectiveHitTechnique),beastCapture,followerEntry,followerDefenseResults,virtualFollowerDefense,followerBundleOptions:bundleOptions,followerBundle,followerAttackOptions:followerAttackOptions(state,viewerId),followerDefenseOptions:hasPriority&&active?.kind==='normal-defense'?self.followers.filter(f=>canSelectFollowerDedicated(f.cardInstanceId,self.characterId)).map(f=>({cardInstanceId:f.cardInstanceId})):[],followerPlacementOptions:followerPlacementOptions(self),magicGateTargets:magicGateTargets(state,viewerId),additionalAttackOptions:additionalAttackOptions(state,viewerId),...choices,groupDefenseOptions,techniqueDecision,additionalAttack,abilityOptions,reactionTargetAbilityId:activeAbility?.stage==='declaration'&&!activeAbility.canceled?activeAbility.id:null,lifetimeDecision,outcome:state.outcome?structuredClone(state.outcome):null,individualResults:{...state.individualResults},lifecycleDecision,lifecycleAbilities, rulesetVersion: state.rulesetVersion, revision: state.revision, phase: state.phase, seatOrder: [...state.seatOrder], turnSeat: state.turnSeat,
    pending: state.pending ? { kind: state.pending.kind, actorId: state.pending.actorId } : null,
    deckCount: state.deck.length, discard: [...state.discard], distances: Object.fromEntries(state.seatOrder.map(a => [a, Object.fromEntries(state.seatOrder.filter(b => a !== b).map(b => [b, state.distances[a]![b]!]))])), distanceMarkers:Object.values(state.distanceMarkers??{}).map(marker=>({...marker})), players,
