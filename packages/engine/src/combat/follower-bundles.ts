...
export interface FollowerBundle {id:string;actorId:string;abilityId:FollowerAbilityId;actionIds:string[];cursor:number;stage:'grant'|'prepare'|'defense'}
...
function canDeclare(s:GameState,actorId:string){const p=s.players[actorId];return !!p&&!s.outcome&&isActive(p)&&canUseCharacterAbility(p)&&!s.windows?.length&&s.phase==='action'&&s.seatOrder[s.turnSeat]===actorId;}
...
function selection(s:GameState,actorId:string,choice:Pick<FollowerAttackSourceChoice,'cardInstanceId'|'dedicated'>):{technique:Technique;sourceZone:'hand'|'followers'}|undefined{
 ...
 if(t.school==='magic'&&hasStatus(p,'silenced')||t.attributes.includes('白')&&getCharacter(p.characterId)?.restrictions.includes('白技使用不可'))return;
...
export function transitionFollowerBundle(state:GameState,input:GameInput):TransitionResult|undefined{
 ...
 for(let i=0;i<c.sources.length;i++){
  ...
  s.resolution.push(choice.cardInstanceId);const actionId=`a-${s.nextEventId++}`;const stats=gameStats(s,actor.id);
  ...
  const a:ActionFrame={followerBundleId:id,sourceZone:resolved.sourceZone,id:actionId,eventId:actionId,parentWindowId:null,actorId:p.id,cardInstanceId:choice.cardInstanceId,kind:'attack',targetIds:[...choice.targetIds],technique:structuredClone(t),groupId:null,stage:'declaration',checks:checkSpecs.map(c=>c.modifier),checkSpecs,roll:null,canceled:false};
...
export function discardBundle(s:GameState,b:FollowerBundle):void{
 const events=new Set<string>();
 for(const id of b.actionIds){const a=s.actions?.[id];if(!a)continue;events.add(a.eventId);const at=s.resolution.indexOf(a.cardInstanceId);if(at>=0){s.resolution.splice(at,1);s.discard.push(a.cardInstanceId);}delete s.actions![id];}
 for(const id of [...s.reclaimReservations]){const reservation=s.reclaim?.[id];if(!reservation||!events.has(reservation.eventId)||Object.values(s.actions??{}).some(a=>a.eventId===reservation.eventId))continue;s.reclaimReservations.splice(s.reclaimReservations.indexOf(id),1);if(isActive(s.players[reservation.ownerId]!))s.players[reservation.ownerId]!.hand.push(id);else s.discard.push(id);delete s.reclaim![id];}
