...
export function beginPrintedComponents(s:GameState,parent:ActionFrame,ids:readonly string[]):void{
 ...
 for(const source of parent.printedComponents){const p=s.players[parent.actorId]!;p.hand.splice(p.hand.indexOf(source.cardInstanceId),1);s.resolution.push(source.cardInstanceId);recordCardPlayed(s,p.id,source.cardInstanceId,'anytime',parent.targetIds);
