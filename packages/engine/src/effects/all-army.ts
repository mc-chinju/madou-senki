...
export function transitionAllArmy(state:GameState,input:GameInput):TransitionResult|undefined{
 ...
 // 全軍突撃せよ is a 複合 card like the other two; the follower under it is what declares the attack.
 for(const card of [ALL_ARMY,c.followerCardInstanceId]){p.hand.splice(p.hand.indexOf(card),1);s.resolution.push(card);recordCardPlayed(s,p.id,card,card===ALL_ARMY?'combination':'attack',c.targetIds);}
