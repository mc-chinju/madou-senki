...
it.each(godsBloodPhysicalScenarios)('%s saves actual Blood reveal stat checks transfer or death and replays every command once',async scenario=>{
 ...
 const initial=await state(),base=gameStats(initial,'A'),b=gameStats(initial,'B'),permanent=structuredClone(initial.players.A!.permanent);await readySetupAsync(async()=>(await room.stored()).state.game!,id=>send(id,{type:'PASS_SETUP'}));await send('A',{type:'START_TURN'});await send('A',{type:'CHOOSE_DRAW',draw:scenario!=='blood-book'});if(scenario==='blood-book'){await send('A',{type:'PLAY_TURN_CARD',cardInstanceId:'a2-p03-r1c1'});await settle();}
