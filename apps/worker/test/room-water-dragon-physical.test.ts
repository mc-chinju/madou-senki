...
it.each([...waterDragonPhysicalScenarios.map(s=>[s,false] as const),['water-dragon-army',true],['water-dragon-grant-field',true]] as const)('%s canceled=%s persists Water Dragon defense and actual source-clock expiry',async(scenario,canceled)=>{
 ...
 if(m.initial)await send('A',{type:'PLACE_INITIAL_FOLLOWER',cardInstanceId:m.card});await send('A',{type:'PASS_SETUP'});for(const cardInstanceId of m.pair??[])await send('B',{type:'PLACE_INITIAL_FOLLOWER',cardInstanceId});for(const id of ['B','C','D'])await send(id,{type:'PASS_SETUP'});await send('A',{type:'START_TURN'});await send('A',{type:'CHOOSE_DRAW',draw:false});
