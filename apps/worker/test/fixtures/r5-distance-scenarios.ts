...
export function makeMaaiScenario(name:MaaiScenarioName,players:{id:string;name:string}[]):GameState {
 ...
 readySetup(()=>s,id=>act(id,{type:'PASS_SETUP'}));act(a,{type:'START_TURN'});act(a,{type:'CHOOSE_DRAW',draw:false});s.distances[a]![b]=s.distances[b]![a]='near';s.events=[];
...
function makeExchangeScenario(name:'r5-distance-approach'|'r5-distance-withdrawal',players:{id:string;name:string}[]):GameState {
 ...
 if(withdraw)s.distances[a]![b]=s.distances[b]![a]='near';readySetup(()=>s,id=>act(id,{type:'PASS_SETUP'}));act(a,{type:'START_TURN'});act(a,{type:'CHOOSE_DRAW',draw:false});s.events=[];
