...
export function makeLiaPrayerScenario(name:LiaPrayerScenario,players:{id:string;name:string}[]):GameState {
 ...
 for(const p of players)act(p.id,{type:'PASS_SETUP'});act(a,{type:'START_TURN'});act(a,{type:'CHOOSE_DRAW',draw:false});act(a,{type:'ATTACK',cardInstanceId:first,targetIds:[name==='lia-prayer-fatal'?b:c],dedicated:false});const offered=()=>s.windows?.at(-1)?.kind==='effect-level'&&s.windows.at(-1)!.participants[s.windows.at(-1)!.cursor]===b;until(offered);if(name!=='lia-prayer-second')return s;
