...
export default {
  async fetch(request: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    try {
      ...
      if (path === '/api/sessions' && request.method === 'POST') return await createSession(request, env.DB);
      if (path === '/api/sessions/current' && request.method === 'GET') return json(await requireSession(request, env.DB));
