...
export function ActionSummary({ action, names }: { action: PlayerView['currentAction']; names: Record<string, string> }) {
  ...
  return <section className="panel action-summary" aria-label="現在の行動"><h2>現在の行動</h2>
    ...
    {action.source !== 'ability' ? <p>{action.technique.school === 'warrior' ? '戦士技' : action.technique.school === 'magic' ? '魔法技' : '技'} · 属性 {action.technique.attributes.join('・')}</p> : null}
    {action.source !== 'ability' ? <p>射程 {rangeNames[action.technique.range] ?? '—'} / 使用値 {action.technique.useLevel}
