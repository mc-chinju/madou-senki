...
export function Board({room,actorId,disabled:connectionDisabled,send}:{room:RoomView;actorId:string;disabled:boolean;send:(command:Command)=>boolean}){
 ...
 return <main id="main-content" className="board"><header className="board-header"><div><p className="eyebrow">{phaseNames[view.phase]??view.phase}</p><h1>戦場</h1></div><div>山札 {view.deckCount} · <button className="card-link" aria-haspopup="dialog" onClick={()=>setShowOwnDiscard(true)}>捨て札 {view.discardCount}</button></div></header>
 <OwnDiscardDialog ids={view.self.discardedCardInstanceIds} open={showOwnDiscard} onClose={()=>setShowOwnDiscard(false)} onInspect={setInspect}/>
