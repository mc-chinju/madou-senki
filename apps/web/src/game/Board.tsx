...
export function Board({room,actorId,disabled:connectionDisabled,send}:{room:RoomView;actorId:string;disabled:boolean;send:(command:Command)=>boolean}){
 ...
 const refocus=useRef<{kind:'place';cardId:string}|{kind:'ready'}|null>(null);
 useEffect(()=>{const wanted=refocus.current;if(!wanted)return;
  if(wanted.kind==='place'?view.self.hand.includes(wanted.cardId):view.phase==='setup'&&!view.pending?.readyIds.includes(view.self.id))return;
