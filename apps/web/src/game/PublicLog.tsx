...
function eventText(view:PlayerView,event:LogView,onInspect:Inspect):ReactNode{
 ...
 switch(event.type){
  ...
  case 'CARD_PLAYED':return opponents&&directedUses.has(event.use!)
   ...
   :<><CardLink id={event.cardInstanceId} onInspect={onInspect}/>を{useNames[event.use!]??''}に使いました{targets}</>;
...
export function PublicLog({view,onInspect}:{view:PlayerView;onInspect:Inspect}){
 ...
 useEffect(()=>{const el=list.current;if(el&&followingRef.current){pin(el);setSeenCount(count);}},[count,order]);
 ...
 const onScroll=()=>{const el=list.current;if(!el)return;const atEnd=orderRef.current==='newest'?el.scrollTop<24:el.scrollHeight-el.scrollTop-el.clientHeight<24;if(atEnd!==followingRef.current)follow(atEnd);};
 ...
 return <section className="panel log" aria-label="公開ログ"><div className="section-title log-title"><h2>戦記</h2>
   <button type="button" className="secondary" aria-pressed={order==='newest'} onClick={flip}>{order==='newest'?'古い順にする':'新しい順にする'}</button>
   {!following?<button type="button" className="secondary" onClick={latest}>{unread?`最新へ（新着${unread}件）`:'最新へ'}</button>:null}</div>
  <div className="log-scroll" ref={list} onScroll={onScroll} tabIndex={0} role="region" aria-label="戦記の全件"><div ref={content}>{sections.map(section=><section key={section.key} aria-label={section.heading}><h3>{section.heading}</h3>
