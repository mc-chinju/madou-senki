...
管理上のgoalも `paused` に変更済み。ユーザーの停止依頼に従い、サイド会話からローカルCodex app-serverの正式な `thread/goal/set` APIで対象本スレッドだけを一時停止し、保存状態でも確認した。目標・使用量は維持しており、complete・blockedには変更していない。本スレッドの終了イベントも確認済み。画面側で追加の停止操作をする必要はない。
...
- 引き継ぎ作成コミットは `e58f4c1`（親は上記検証済みHEAD）。その後、この文書と計画のgoal停止状態だけを修正した。最終HEADの自己参照hashは埋め込まず、`git log -1 --format='%H %s'` で取得する。
...
1. 別セッションでこの文書と計画を読み、ユーザーからの明示的な再開指示を確認する。新セッションで同じ計画を引き継ぐ。旧スレッドのgoalはpausedのままにして二重実行を避ける。旧スレッド自体へ戻る場合だけ、そのgoalを再開する。
