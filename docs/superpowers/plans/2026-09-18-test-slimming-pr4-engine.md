...
- `owned-reclaim` の個別版 3件は完全重複ではなかったため復元した。現在の decision に対する偽造 claim の拒否と状態不変、侍大将と妖撃破山剣による従者死亡、`reclaimUsage` 未生成の検証を元のまま保持する。統合後も engine の実行件数は 7,485件で、削除した `it` はない。
