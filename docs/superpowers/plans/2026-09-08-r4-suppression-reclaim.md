...
| `All-pass zero base extra and unlimited worlds have identical public transcripts` | Parameterize no right/base/extra/unlimited while holding public source/presence fixed; use identical explicit PASS sequence for every public seat. Compare non-owner command receipts, revision/event IDs, source zone until final pass, next parent window and final discard; no server-generated auto-pass/shorter sequence. |
...
| `Two explicit claimants share one card and saved finite priority` | First user claim and second printed-right claim on same ID; first decline enables next; first take removes later claim; ordinary ownership of same name creates no third claim. 2026-09-10: actual Lester Courage base/printed alternatives pass (2 cases, [evidence](../../operations/evidence/2026-09-10-r4-lester-two-rights.md)); these rights share one seat. Decline currently advances that whole seat, so the exact decline-to-next-right requirement remains unproven. |
...
- [ ] Actual normal bow and a legally reachable bow counter are canonical evidence. A bundle source with Fury installed through helper-only ownership is structural evidence unless a real route grants that ability combination; explicitly classify it. Never manufacture an unsupported legal route just to tick coverage.
