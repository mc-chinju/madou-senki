...
- **跳ね返しは、割り込んだその場で記録する。** 反射は解決を子グループへ渡して `return` するため、全ヒットが応じ済みになった従者は二度と `summarize` に戻らず、`FOLLOWER_DEFENDED` が1件も残らなかった（単発の攻撃では必ずこうなる）。反射の分岐で記録し、`summarize` 側は反射済みの従者を重ねて記録しない（1人の従者につき1行）
