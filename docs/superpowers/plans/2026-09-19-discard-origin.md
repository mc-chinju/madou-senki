...
- [x] `view.ts`: `self.discardedCardInstanceIds`（自分が捨てて今も捨て札にある札、捨てた順）を足す。他者には `discardCount` のみ（現状維持）
- [x] 試験（先に書く）: 他視点にこの配列が出ない、山札へ戻したら消える、回収で手札に戻したら消える、死亡で捨てた手札が本人には見える、bot の全試合で他者の札IDが混ざらない
- [x] 画面: 見出しの「捨て札 N」をボタンにし、ダイアログで自分の捨て札を出す。新しい順と、同名をまとめた表示を切り替えられる。札名から既存の `CardDialog` を開ける。キーボードで開閉できる
- [x] `apps/web/test` に表示試験
