...
- [ ] 歌う船 `semantic/defense/not-earth-warrior`: 実地属性戦士producerが未発見。既存合成consumer回帰は成功したが、実カード成立証跡へ昇格せずpendingを維持。従者側の地属性を攻撃へ流用しない。
...
- [ ] 有翼族 `semantic/defense/not-earth-warrior`: 歌う船と同じく実地属性戦士producerの成立証跡が不足。従者の地属性を攻撃へ流用せずpendingを維持。
