...
小規模な友人同士のテストから開始する。最初は匿名ゲストセッションと表示名、招待リンクで十分。恒久アカウント、フレンド、ランキング、レーティング、観戦、チャット、音声通話、通知、日をまたぐ非同期対戦は後段へ分ける。招待リンクは作成して画面に出し、ユーザー自身が共有する。
...
- ゲストには署名/不透明なセッションCookieを発行し、HttpOnly・Secure・SameSiteとOrigin検証を使う。Cookieを削除した後の席復旧は初期版の対応外で、参加時に案内する。
