...
3. 当事者間の踏み込み・間合いは明示的に交互回答する。第三者介入の窓では、起点と席順が公開された順番で優先権を渡す。パスだけは対象者が優先権の前に出せる（パス先出し）。実介入の順番は変えない。
...
先出しパスの有無と時刻は公開される。卓上で「パス」と先に言うのと同じ扱いとし、手札や能力の有無から自動で入るパスとは区別する。「この行動は任せる」は本人が行動ごとに明示し、手札によらず同じ動きをするので前者に含める。停止中の席は公開情報として公開窓の回答者から外す（回収回答を除く）。
...
3. revisionと反応窓の世代を検証し、古い画面からの操作なら再同期を求める。窓世代とsetupラウンドが同じなら、他席のパス・配置によるrevision差は受理する。
...
同時入力は正常系として扱う。クライアントは操作の基準として、反応窓があれば `{windowId, windowRevision}`（窓世代）、初期配置中は `{windowId: "setup-<ラウンド番号>", windowRevision: 0}`、それ以外は基準なしを送る。サーバーは、基準がある操作なら `expectedRevision` が現在の revision より古くても、基準が現在の基準と一致する限り受理し、エンジンの合法性検査に委ねる。基準が一致しない、または基準のない場面（通常の手番操作・ロビー）では従来どおり revision の完全一致を要求する。クライアントのACK判定も `revision > expectedRevision` に緩める。これで、他席のパスや初期配置と同時に送った操作が、それだけの理由で弾かれることはない。
...
| 山札 | 枚数 | 枚数 |
| 捨て札 | 自分が捨てた札の一覧と、全体の枚数 | 枚数のみ |
| 行動候補・任意能力 | 本人に合法な候補 | 内容・存在を送らない |
| 公開ログ | 公開された事実を対戦開始から全件 | 同じ公開された事実を対戦開始から全件 |
| 決着後（`outcome` 確定） | 全員の正体・手札・伏せた詠唱・裏向き従者・捨て札・山札 | 同じ |

決着後の全公開は原文に規定が無い。卓上では決着したら手札を見せ合って感想戦をするため、そのためのオンライン補完として裁定する。山札まで開示するのは、引きの運を後から検証できるようにするため。開示は `outcome` が確定した瞬間より前には一切行わない（`outcome` が入るまでは上の各行のとおり）。対戦中の記録（戦記）はこの開示で書き換えない。その時点で見えたものだけが記録に残る原則は決着後も変わらない。

### 戦記（公開ログ）の範囲

「その場で見えたものは戦記で見返せる。誰も見ていないものは見えない」にそろえる。公開の線は「その時点で表向きに場へ出たか」で決め、記録はその時点の内容で固定する。エンジンが公開事実を構造化イベントとして記録し、`logView` の許可リストで視点ごとに投影する。文章はエンジンで作らず、画面が組み立てる。捨て札の中身は決着前の `viewFor` の段階で落とす（決着後は上の表の最終行のとおり開示する）。
