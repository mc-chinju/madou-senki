...
test('S07 Lia reuses the same returned prayer on a later independent attack across browser reloads',async({browser,request})=>{
 ...
 await passUntil(table,views,g=>!g.activeWindow,300);await bp.reload();expect(own().self.hand.filter(id=>id===prayer)).toHaveLength(1);expect(own().reservedCards).not.toContain(prayer);expect(game().discard).not.toContain(prayer);
