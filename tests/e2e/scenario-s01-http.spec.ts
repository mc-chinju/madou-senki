...
test('S01 authenticated HTTP snapshots match all four WebSocket priority views and reject outsiders',async({browser,request})=>{
 ...
 const outsider=await browser.newContext({baseURL:'http://localhost:8787'});try{const page=await outsider.newPage();await page.goto('/');await page.getByLabel('表示名').fill('外部');await page.getByRole('button',{name:'はじめる',exact:true}).click();await expect(page.getByRole('heading',{name:/ようこそ/})).toBeVisible();const result=await page.evaluate(async roomId=>{const r=await fetch(`/api/rooms/${roomId}/snapshot`);return {status:r.status,body:await r.json()};},table.roomId);expect(result).toEqual({status:403,body:{error:'FORBIDDEN'}});}finally{await outsider.close();}
