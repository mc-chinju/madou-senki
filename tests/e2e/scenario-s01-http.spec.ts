...
import {observe,signIn,tableFixture} from './helpers.js';
test('S01 authenticated HTTP snapshots match all four WebSocket priority views and reject outsiders',async({browser,request,baseURL})=>{
 ...
 const outsider=await browser.newContext({baseURL});try{const page=await outsider.newPage();await signIn(outsider,'外部');await page.goto('/');await expect(page.getByRole('heading',{name:/ようこそ/})).toBeVisible();const result=await page.evaluate(async roomId=>{const r=await fetch(`/api/rooms/${roomId}/snapshot`);return {status:r.status,body:await r.json()};},table.roomId);expect(result).toEqual({status:403,body:{error:'FORBIDDEN'}});}finally{await outsider.close();}
