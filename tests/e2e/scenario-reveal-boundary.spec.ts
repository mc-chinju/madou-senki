...
async function verifyScenario(browser:Browser,request:APIRequestContext,scenario:'canonical-S15-before'|'canonical-S15-after'|'canonical-S16'){
 ...
 await passUntil(table,views,g=>!g.activeWindow,300);await page.reload();const game=views.get(a)!.game!;expect(game.players[b]!.revealed).toBe(true);expect(game.players[b]!.damage).toBe(scenario==='canonical-S15-after'?4:0);expect(game.discard.filter(id=>id===(zero?'a2-p08-r3c3':'a2-p24-r1c2'))).toHaveLength(1);
