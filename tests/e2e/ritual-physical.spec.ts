...
for(const scenario of ritualPhysicalScenarios)test(`${scenario} actual ritual controls private gift public transformation and optional awakening survive reload`,async({browser,request})=>{
 ...
 try{
  ...
  if(scenario.includes('gift')){
   ...
   const aHand=[...own(a).hand],bHand=[...own(b).hand],revealed=game().players[b]!.revealed,deck=game().deckCount,discard=[...game().discard],windowId=game().activeWindow?.windowId,phase=game().phase;
  ...
  if(scenario==='ritual-fate'){await until(()=>game().activeWindow?.pendingActorId===d);const reaction=dp.getByRole('complementary',{name:'現在の判断'});await reaction.getByRole('combobox',{name:'使うカード',exact:true}).selectOption('a2-p02-r2c3');await click(reaction.getByRole('button',{name:'割り込みを使う',exact:true}));await until(()=>!game().activeWindow);expect(own(a).damage).toBe(before.damage);expect(own(a).characterId).toBe('c2-p05-r1c1');expect(game().discard).toContain(ritualCard);return;}
  ...
  expect(own(b).faction).toBe(sub?'ヴァンミール':'EVIL');expect(own(d).faction).toBe(sub?'ヴァンミール':'EVIL');expect(game().players[c]!.presence).toBe(conspiracy?'exited':'active');expect(game().discard.filter(id=>id===ritualCard)).toHaveLength(1);expect(game().phase).toBe('hand-adjustment');expect(own(a).hand).toEqual(before.hand.filter(id=>id!==ritualCard));await page.reload();expect(own(a).characterId).toBe('c2-p07-r1c2');expect(errors).toEqual([]);
